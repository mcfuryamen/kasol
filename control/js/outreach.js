...
const countBy = (k) => contacts.filter((c) => c.status === k).length;

/** Chip rail — filter + statistik dalam satu elemen (signature layar ini) */
function renderChips() {
  const host = document.getElementById('outreachChips');
  if (!host) return;
  const chips = [{ key: '', short: 'Semua', tone: 'gray', n: contacts.length }]
    .concat(OUTREACH_STAGES.map((s) => ({ ...s, n: countBy(s.key) })));
    ...
    return `
      <div class="oc-row" data-id="${escapeHtml(c.id)}" role="button" tabindex="0" aria-label="${escapeHtml(c.nama)}, ${escapeHtml(sm.label)}">
        ...
        <div class="oc-side">
          ...
          <button type="button" class="oc-wa" aria-label="WhatsApp ${escapeHtml(c.nama)}" title="Buka WhatsApp" onclick="event.stopPropagation(); openOutreachWa('${escapeHtml(c.id)}')">💬</button>
        </div>
      </div>`;
  }).join('');
...
let waCtx = null; // { id, group, variant }

/** Buka modal template chat — pesan digenerate sesuai konteks, bisa diedit */
window.openOutreachWa = function (id) {
  const c = contacts.find((x) => x.id === id);
  if (!c) return;
  const group = templateGroup(c);
  waCtx = { id, group, variant: 0 };
  document.getElementById('outreachWaHint').textContent =
    group === 'intro' ? 'Pesan perkenalan — kontak ini belum pernah dihubungi.'
    : group === 'followup' ? 'Pesan follow-up — kontak ini sudah pernah dihubungi.'
    : group === 'minat' ? 'Pesan lanjutan — kontak ini sudah bilang minat.'
    : 'Pesan after-sales — kontak ini sudah beli.';
  document.getElementById('outreachWaText').value = fillTemplate(WA_TEMPLATES[group][0], c);
  document.getElementById('sheetOutreachWa')?.classList.add('open');
};
...
window.regenerateOutreachWa = function () {
  ...
  document.getElementById('outreachWaText').value = fillTemplate(list[waCtx.variant], c);
...
window.resetOutreachWa = function () {
  ...
  const c = contacts.find((x) => x.id === waCtx.id);
  if (!c) return;
  document.getElementById('outreachWaText').value = fillTemplate(WA_TEMPLATES[waCtx.group][waCtx.variant], c);
};

window.closeOutreachWa = function () {
  waCtx = null;
  closeSheet('sheetOutreachWa');
};

/** Buka WhatsApp dengan pesan terisi (wa.me?text=) + tandai terkirim */
...
window.openOutreachDetail = function (id) {
  ...
  detailId = id;
  renderSheetBody(c);
  document.getElementById('sheetOutreachDetail')?.classList.add('open');
};
...
function renderSheetBody(c) {
  ...
  document.getElementById('outreachDetailBody').innerHTML = `
    ...
    <div class="btn-block-row">
      ...
      <button type="button" class="btn btn-primary" onclick="openOutreachWa('${esc(c.id)}')">💬 WhatsApp</button>
    </div>
  `;
